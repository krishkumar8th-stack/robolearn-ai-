...
export interface RoboticsProject { id: string; title: string; tagline: string; difficulty: 'Beginner' | 'Intermediate' | 'Advanced'; category: string; imageUrl?: string; estimatedHours: number; xpReward: number; prerequisites: string[]; componentsRequired: string[]; circuitOverview: string; theory: string; steps: ProjectStep[]; completeCode: string; programmingLanguage: string; simulationConfig: { components: string[]; initialObstacleDistance?: number }; }
export interface Achievement { id: string; title: string; description: string; category: 'learning' | 'coding' | 'robotics' | 'streak'; badgeIcon: string; xpValue: number; unlockedAt?: string; }
export type SimulationActionType = 'LED_SET' | 'LED_BLINK' | 'SERVO_SET' | 'SERVO_SWEEP' | 'MOTOR_SPEED' | 'ROBOT_MOVE' | 'ROBOT_TURN' | 'ROBOT_STOP' | 'BUZZER_TONE' | 'ULTRASONIC_PING' | 'OBSTACLE_DETECT';
export interface SimulationAction { type: SimulationActionType; componentId?: string; pin?: number | string; state?: 'ON' | 'OFF' | boolean; angle?: number; speed?: number; direction?: 'FORWARD' | 'BACKWARD' | 'LEFT' | 'RIGHT'; duration?: number; frequency?: number; distance?: number; delayMs?: number; }
export interface SimulationState { isRunning: boolean; isPaused: boolean; speedMultiplier: number; leds: Record<string, { on: boolean; brightness: number }>; servos: Record<string, { angle: number }>; motors: Record<string, { speed: number; direction: 'FORWARD' | 'BACKWARD' | 'STOP' }>; robot: { x: number; z: number; rotationY: number; speed: number; status: 'IDLE' | 'MOVING' | 'TURNING' | 'STOPPED_OBSTACLE' }; obstacle: { x: number; z: number; distanceToRobot: number }; serialLogs: { timestamp: number; text: string; type: 'info' | 'output' | 'error' }[]; }
export interface AIChatMessage { id: string; sender: 'user' | 'assistant' | 'system'; content: string; codeSnippet?: string; language?: string; components?: string[]; simulationActions?: SimulationAction[]; timestamp: string; }
export interface AICodeGenerationResult { language: string; code: string; requiredComponents: string[]; wiring: { from: string; to: string; note: string }[]; explanation: string; functionsUsed: { name: string; purpose: string }[]; possibleErrors: string[]; simulationSupported: boolean; simulationActions?: SimulationAction[]; challenge?: { title: string; prompt: string }; }
export interface AIDebugResult { problem: string; cause: string; solution: string; correctedCode: string; explanation: string; confidence: 'High' | 'Medium' | 'Low'; preventionTips: string[]; }
