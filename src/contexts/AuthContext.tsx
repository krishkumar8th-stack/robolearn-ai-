...
function validateAuthResponse(response: { token?: unknown; user?: unknown }) {
  if (!response || typeof response.token !== 'string' || !response.token.trim()) throw new Error('Login succeeded on the server, but no valid session token was returned. Please try again.');
  if (!response.user || typeof response.user !== 'object') throw new Error('Login succeeded, but the server returned an invalid user response.');
}
...
function syncDashboardStats(user: User | null) {
  ...
  const setStat = (label: string, value: string) => { const labelNode = progressNodes.find(node => node.textContent?.trim() === label); const valueNode = labelNode?.previousElementSibling as HTMLElement | null; if (valueNode && valueNode.textContent !== value) valueNode.textContent = value; };
  setStat('Overall Progress', `${overallProgress}%`); setStat('Current Course', String(activeCourses)); setStat('Lessons Completed', String(completedLessons)); setStat('Challenges Solved', String(completedChallenges)); setStat('Projects Completed', String(completedProjects)); setStat('XP Earned', String(xp)); setStat('Day Streak', String(streak));
  const sidebarProgress = progressNodes.find(node => node.textContent?.trim() === 'Your Progress')?.parentElement;
  if (sidebarProgress) {
    const percentNode = Array.from(sidebarProgress.querySelectorAll('span')).find(node => /^\d+%$/.test(node.textContent?.trim() || '')); if (percentNode) percentNode.textContent = `${overallProgress}%`;
    const levelNode = Array.from(sidebarProgress.querySelectorAll('p')).find(node => /^Level \d+ - /.test(node.textContent?.trim() || '')); const levelName = level >= 5 ? 'Builder' : level >= 3 ? 'Explorer' : 'Beginner'; if (levelNode) levelNode.textContent = `Level ${level} - ${levelName}`;
    const xpNode = Array.from(sidebarProgress.querySelectorAll('p')).find(node => /XP$/.test(node.textContent?.trim() || '')); if (xpNode) xpNode.textContent = `${xp} XP`;
    const progressPath = sidebarProgress.querySelector('path[stroke-dasharray]') as SVGPathElement | null; if (progressPath) progressPath.setAttribute('stroke-dasharray', `${overallProgress}, 100`);
  }
  root.setAttribute('data-user-id', user.id); root.setAttribute('data-learned-components', String(learnedComponents));
}

export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [user, setUser] = useState<User | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const refreshUser = async () => { const token = getToken(); if (!token) { setUser(null); setIsLoading(false); return; } setIsLoading(true); try { const res = await api.getMe(); if (!res?.user) throw new Error('Invalid session response.'); setUser(res.user); } catch { localStorage.removeItem(TOKEN_KEY); setUser(null); } finally { setIsLoading(false); } };
