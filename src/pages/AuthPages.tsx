...
import { Bot, Lock, Mail, Sparkles, AlertCircle, Eye, EyeOff, Loader2 } from 'lucide-react';
...
export const LoginPage: React.FC = () => {
  ...
  const [email, setEmail] = useState('maker@roblearn.ai');
  const [password, setPassword] = useState('maker123');
    ...
    <div className="flex min-h-[80vh] flex-col items-center justify-center bg-slate-50 p-4 text-slate-900 dark:bg-[#070b14] dark:text-slate-100">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900 sm:p-8">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Sign in to continue to your RoboLearn AI workspace</p>
        </div>
        ...
        <div className="mt-6 space-y-3 border-t border-slate-200 pt-4 dark:border-slate-800">
          <button type="button" onClick={handleDemo} disabled={isLoading} className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 py-2.5 text-xs font-bold text-cyan-600 transition hover:bg-slate-100 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800 dark:text-cyan-300 dark:hover:bg-slate-700">
            <Sparkles className="h-3.5 w-3.5" /> Use Demo Account
          </button>
          <p className="text-center text-xs text-slate-500 dark:text-slate-400">Don't have an account? <Link to="/register" className="font-bold text-cyan-500 hover:underline">Create one</Link></p>
        </div>
      </div>
    </div>
  );
...
export const RegisterPage: React.FC = () => {
  ...
  return (
    <div className="flex min-h-[80vh] flex-col items-center justify-center bg-slate-50 p-4 text-slate-900 dark:bg-[#070b14] dark:text-slate-100">
      <div className="w-full max-w-md rounded-3xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-slate-800 dark:bg-slate-900 sm:p-8">
        <div className="mb-6 text-center"><div className="mx-auto mb-3 flex h-12 w-12 items-center justify-center rounded-2xl border border-cyan-500/30 bg-cyan-500/10 text-cyan-500"><Bot className="h-6 w-6" /></div><h2 className="text-2xl font-black">Create Maker Account</h2><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Create an account and start learning with RoboLearn AI</p></div>
        {error && <div role="alert" className="mb-4 rounded-xl border border-rose-500/30 bg-rose-500/10 p-3 text-xs text-rose-500 dark:text-rose-300">{error}</div>}
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input required autoComplete="username" value={username} onChange={e => setUsername(e.target.value)} placeholder="Username" className="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm outline-none focus:border-cyan-500 dark:border-slate-700 dark:bg-slate-950" />
